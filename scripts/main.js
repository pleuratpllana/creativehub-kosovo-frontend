...
const lines = document.querySelectorAll(".vertical-line-fill");
...
function showLine() {
  const breakingPoint = (window.innerHeight / 5) * 4;

  lines.forEach((line) => {
    const getLine = line.getBoundingClientRect().top;

    if (getLine < breakingPoint) {
  ...
  var form_submit = function (e) {
    ...
    if (validate_form()) {
      ...
      var serialized = _form_serialize(
        document.getElementById("_form_404_")
      ).replace(/%0A/g, "\\n");
...
new Glide(".glide-hiring-results", configHiringResults).mount();

const configMoreResults = {
  type: "carousel",
  perView: 1,
  animationDuration: 1000,
  breakpoints: {
...
new Glide(".glide-more-results", configHiringResults).mount();
...
window._load_script = function (url, callback, isSubmit) {
  ...
  var submitButton = document.querySelector("#_form_598_submit");
  ...
  script.onerror = function () {
    if (isSubmit) {
      if (script.src.length > 10000) {
        ...
        _show_error("598", "Sorry, your submission failed. Please try again.");
      }
...
(function () {
  ...
  var form_to_submit = document.getElementById("_form_598_");
  ...
  var form_submit = function (e) {
    ...
    if (validate_form()) {
      ...
      var submitButton = e.target.querySelector("#_form_598_submit");
      ...
      var serialized = _form_serialize(
        document.getElementById("_form_598_")
      ).replace(/%0A/g, "\\n");
      ...
      async function submitForm() {
        ...
        const response = await fetch(
          "https://akoskarova.activehosted.com/proc.php?jsonp=true",
          {
            headers: {
...
document.getElementsByName("calculator-rati").forEach((el) => {
  el.addEventListener("change", (e) => {
    ...
    updateActiveState(e.target, ".label-instalments");
    canCalculate();
    calculate();
  });
});
...
document.getElementsByName("academy-price").forEach((el) => {
  el.addEventListener("change", (e) => {
    ...
    updateActiveState(e.target, ".label-academy");
    canCalculate();
    calculate();
  });
});
...
const calculate = () => {
  const price = Math.round(calculator.type / calculator.rati);
  calculatorPrice.innerHTML = price;
};

const updateActiveState = (target, selector) => {
...
//       counter("count8", 0, 84, 5500);

//       counter("count9", 0, 87, 3500);

//       counter("count10", 0, 65, 4500);
...
// observer2.observe(document.querySelector(".percent-bottom1"));
  ...
  script.onerror = function () {
    if (isSubmit) {
      if (script.src.length > 10000) {
        ...
        _show_error("259", "Sorry, your submission failed. Please try again.");
      }
    }
  };
...
(function () {
  ...
  var form_submit = function (e) {
    ...
    if (validate_form()) {
      ...
      var serialized = _form_serialize(
        document.getElementById("_form_259_")
      ).replace(/%0A/g, "\\n");
...
// Real Projects Section Video
document.addEventListener("DOMContentLoaded", function () {
  const playButton = document.querySelector(".play-button");
  if (playButton) {
    playButton.addEventListener("click", function () {
      const container = this.closest(".col-lg-4");
